...
train <- read.csv('data/transformed_data_val.csv')
library(DMwR)
...
corrplot.mixed(as.matrix(above_50))
...
ggplot(top_countries,aes(x=variable,y=value,fill=variable)) + 
  ...
  ggtitle('Scores of Top Countries') + facet_wrap(~Country.Name) +
  geom_line(stat='identity',aes(y=Mean.Rank,group=1), color = 'red',size=1.5)
...
p <- ggplot(top_countries) + 
  ...
  ggtitle('Scores of Top Countries') +
...
gganimate(p)
gganimate(p,filename = 'Plots/Top_Features_scores.gif')
...
ani.options(interval=0.5)
gganimate(p2)
gganimate(p2,filename = 'Plots/Top_Features2_scores.gif')
